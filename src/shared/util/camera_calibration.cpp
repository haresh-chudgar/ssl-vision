...
void CameraParameters::field2image_i2f(
...
double CameraParameters::calc_chisqr(
  ...
  for (; it_p_f != p_f.end(); it_p_f++, it_p_i++)
  {
    ...
    field2image_i2f(*it_p_f, proj_p, p);
    ...
    for (; ls_it != calibrationSegments.end(); ls_it++) {
      ...
      for (; imgPts_it != (*ls_it).imgPts.end(); imgPts_it++) {
        ...
        if (imgPts_it->second) {
          ...
          field2image_i2f(alpha_point, proj_p, p);
...
void CameraParameters::do_calibration(int cal_type) {
  ...
  if(cal_type == FOUR_POINT_INITIAL) {
    initialCalibration(p_f, p_i, cal_type);
  } else {
  ...
  while (!stop_optimization) {
    ...
    for (; it_p_f != p_f.end(); it_p_f++, it_p_i++) {
      ...
      for (; it != p_to_est.end(); it++) {
        ...
        field2image_i2f(*it_p_f, proj_p_diff, p_diff);
...
void CameraParameters::fullCalibration(std::vector<GVector::vector3d<double> > &p_f, std::vector<GVector::vector2d<double> > &p_i, int cal_type) {
